...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // 일별 분석 데이터 조회
    const analytics = await prisma.floatingButtonAnalytics.findMany({
      where: {
        buttonId: id,
        date: {
          gte: startDateObj,
    ...
    // 전체 통계
    const totalStats = await prisma.floatingButtonAnalytics.aggregate({
      where: {
        buttonId: id,
        date: {
          gte: startDateObj,
    ...
    console.error('Error fetching floating button analytics:', error)
    return NextResponse.json(
      { error: '플로팅 버튼 분석 데이터를 불러오는데 실패했습니다.' },
      { status: 500 }
    )
  }
}