...
export default function SurveyTable({ requests, total, pages, currentPage, searchParams }: Props) {
  const router = useRouter()
  const [search, setSearch] = useState(searchParams.search || '')
  const [status, setStatus] = useState(searchParams.status || '')
  const [selectedRequest, setSelectedRequest] = useState<SurveyRequest | null>(null)

  const handleSearch = () => {
    const params = new URLSearchParams()
    if (search) params.set('search', search)
    if (status) params.set('status', status)
    router.push(`/admin/cooperation/survey?${params.toString()}`)
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">설문·인터뷰 요청 관리</h1>
        ...
        {requests.length === 0 ? (
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                <th className="px-6 py-4 text-left text-sm font-medium text-gray-500">요청자</th>
                <th className="px-6 py-4 text-left text-sm font-medium text-gray-500">설문기간</th>
                <th className="px-6 py-4 text-left text-sm font-medium text-gray-500">문항/시간</th>
                ...
                <tr key={req.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4">
                    ...
                    <p className="text-sm text-gray-500">
                      {req.requesterType === 'INDIVIDUAL' ? '개인' : '기관'} · {req.email}
                    </p>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <span className="px-2 py-1 bg-gray-100 text-gray-700 text-sm rounded">
                      {req.questionCount}문항
                    </span>
                    <span className="ml-2 text-sm text-gray-500">{req.estimatedTime}분</span>
                  </td>
                  <td className="px-6 py-4">
