...
export default function LecturerTable({ requests, total, pages, currentPage, searchParams }: Props) {
  ...
  const handleSearch = () => {
    ...
    router.push(`/admin/cooperation/lecturer?${params.toString()}`)
  }
  ...
  const getFirstSchedule = (schedules: any) => {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">강사요청 관리</h1>
        ...
        {requests.length === 0 ? (
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                <th className="px-6 py-4 text-left text-sm font-medium text-gray-500">강연주제</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {requests.map((req) => {
                const firstSchedule = getFirstSchedule(req.schedules)
                return (
                  <tr key={req.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4">
                      <p className="font-medium text-gray-900 truncate max-w-[200px]">{req.topic}</p>
                      <p className="text-sm text-gray-500">{req.method === 'OFFLINE' ? '오프라인' : '온라인'}</p>
                    </td>
                    <td className="px-6 py-4">
                      <p className="font-medium text-gray-900">{req.organization}</p>
                      <p className="text-sm text-gray-500">{req.contactName}</p>
                    </td>
                    <td className="px-6 py-4 text-gray-600">
                      {firstSchedule ? (
                        <>
                          <p>{firstSchedule.date}</p>
                          <p className="text-sm text-gray-500">
                            {firstSchedule.startTime} - {firstSchedule.endTime}
                            {Array.isArray(req.schedules) && req.schedules.length > 1 && (
                              <span className="text-primary"> 외 {req.schedules.length - 1}건</span>
                            )}
                          </p>
                        </>
                      ) : (
