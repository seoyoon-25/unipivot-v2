...
export default function EditProgramPage() {
  ...
  const { hasDraft, lastSaved, restoreDraft, clearDraft } = useAutoSave({
    key: `program-draft-edit-${slug}`,
    data: { form, sessions },
    enabled: dataLoaded, // 데이터 로드 후에만 자동저장 활성화
  })
  ...
  useEffect(() => {
    const fetchProgram = async () => {
      try {
        // 먼저 slug로 프로그램 ID 조회
        const slugRes = await fetch(`/api/programs/by-slug/${slug}`)
        if (!slugRes.ok) {
        ...
        alert(error.message || '프로그램을 불러올 수 없습니다.')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        const error = await res.json()
        throw new Error(error.error || '프로그램 수정 실패')
      }
      ...
      alert(error.message || '프로그램 수정 중 오류가 발생했습니다.')
    } finally {
      setSaving(false)
    }
  }
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">모집인원</label>
                  <input
                    type="number"
                    value={form.capacity}
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <RichTextEditor
                    ...
                    minHeight="300px"
                    autoSaveKey={`program-edit-content-${slug}`}
                  />
                </div>
                ...
                <div>
                  ...
                  <RichTextEditor
                    ...
                    minHeight="200px"
                    autoSaveKey={`program-edit-schedule-${slug}`}
                  />
                </div>
                ...
                {form.type === 'BOOKCLUB' && (
                  <div>
                    ...
                    <RichTextEditor
                      ...
                      minHeight="200px"
                      autoSaveKey={`program-edit-book-${slug}`}
                    />
                  </div>
                )}
              </div>
