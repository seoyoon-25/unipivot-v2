...
export default function HelpPage() {
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold text-zinc-900 mb-2">도움말</h1>
      <p className="text-zinc-500 mb-8">유니클럽 사용에 필요한 모든 안내</p>
        ...
        {quickLinks.map((link) => {
          ...
          return (
            <Link
              key={link.href}
              href={link.href}
              className="p-4 bg-white rounded-2xl border border-zinc-100 shadow-sm hover:border-blue-200 hover:shadow-sm transition-all"
            >
      ...
      <section>
        <h2 className="text-lg font-semibold text-zinc-900 mb-4">자주 묻는 질문</h2>
          ...
          {faqItems
            ...
            .flatMap((cat) =>
              cat.questions.slice(0, 2).map((q, i) => (
                ...
                >
                  <summary className="p-4 font-medium text-zinc-900 cursor-pointer hover:bg-zinc-50 rounded-xl">
                    {q.q}
                  </summary>
                  <p className="px-4 pb-4 text-zinc-600">{q.a}</p>
                </details>
              ))
            )}
        </div>
