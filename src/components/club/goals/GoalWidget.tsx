  ...
  if (!hasGoal) {
    return (
      <Link
        ...
        className="block bg-white rounded-2xl border border-zinc-100 shadow-sm p-4 hover:border-blue-200 transition-colors"
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <BookOpen className="w-5 h-5 text-zinc-400" />
            <span className="text-sm text-zinc-500">독서 목표를 설정해보세요</span>
          </div>
    ...
    >
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium text-zinc-900 text-sm">{label} 독서 목표</span>
        </div>
