    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="presentation">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl" role="dialog" aria-modal="true" aria-labelledby="add-participant-title">
        ...
        <div className="max-h-64 overflow-y-auto">
          {isSearching ? (
            <div className="p-8 text-center text-zinc-500 text-sm">검색 중...</div>
          ) : results.length === 0 ? (
        ...
        <div className="p-4 border-t border-zinc-200">
          <button
            onClick={onClose}
            className="w-full py-2 border border-zinc-200 rounded-lg text-sm text-zinc-700 hover:bg-zinc-50 transition-colors"
          >
            닫기
          </button>
        </div>
